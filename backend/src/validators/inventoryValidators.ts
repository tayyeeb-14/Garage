...
const inventoryFields = {
  ...
  kmDriven: numberField('KM Driven').pipe(z.number().int().min(0)).optional(),
  ownerCount: numberField('Owner count').pipe(z.number().int().min(0)).optional(),
  location: z.string().trim().max(120).optional(),
  sellerContact: z.string().trim().max(120).optional(),
  verifiedSeller: booleanField,
  isTrending: booleanField,
  offerDetails: z.string().trim().max(200).optional(),
